...
from .parallel_fixer import ParallelFixer, FixTask, FixPriority, FixTaskGenerator, CommonFixes
...
"""
        ...
        @tool(
            "check_frontend",
            "🔍 前端检查 - 运行 TypeScript 类型检查和 ESLint，检测缺失依赖和导入错误",
            {
                "include_eslint": bool,  # 是否包含 ESLint 检查
            }
        )
        async def check_frontend(args: dict[str, Any]) -> dict[str, Any]:
            """运行前端代码检查"""
            include_eslint = args.get("include_eslint", True)

            checker = FrontendChecker(project_path)
            result = await checker.check_all()

            # 生成摘要
            summary = checker.get_error_summary(result.typescript_errors)

            output = {
                "typescript_errors": {
                    "total": len(result.typescript_errors),
                    "critical": summary["by_severity"]["critical"],
                    "high": summary["by_severity"]["high"],
                    "files_affected": summary["files_affected"],
                    "top_errors": list(summary["by_code"].items())[:10],
                    "errors": [e.to_dict() for e in result.typescript_errors[:50]],
                },
                "missing_dependencies": [d.to_dict() for d in result.missing_dependencies],
                "import_errors": result.import_errors[:20],
            }

            if include_eslint:
                output["eslint_errors"] = {
                    "total": len(result.eslint_errors),
                    "errors": result.eslint_errors[:30],
                }
        ...
        @tool(
            "analyze_pm2_logs",
            "📋 PM2 日志分析 - 分析 PM2 错误日志，检测运行时错误、依赖问题、数据库错误等",
            {
                "lines": int,  # 分析多少行日志
                "app_name": str,  # PM2 应用名称
            }
        )
        async def analyze_pm2_logs(args: dict[str, Any]) -> dict[str, Any]:
            """分析 PM2 错误日志"""
            lines = args.get("lines", 500)
            app_name = args.get("app_name", "sillytavern-web")

            analyzer = LogAnalyzer(project_path)
            result = await analyzer.analyze(lines=lines, app_name=app_name)
            ...
            output = {
                ...
                "errors": [e.to_dict() for e in result.errors[:30]],
                "error_patterns": result.error_patterns,
                "recommendations": result.recommendations,
                "actionable_fixes": analyzer.get_actionable_fixes(result),
            }
        ...
        @tool(
            ...
            "⚡ 并行修复 - 多线程并行修复多个代码问题",
            {
                "fix_tasks": str,  # JSON 格式的修复任务列表
                "max_workers": int,  # 最大工作线程数
                "dry_run": bool,  # 是否仅模拟运行
            }
        )
        async def parallel_fix(args: dict[str, Any]) -> dict[str, Any]:
            """并行执行修复任务"""
            max_workers = args.get("max_workers", 4)
            dry_run = args.get("dry_run", False)

            fixer = ParallelFixer(project_path, max_workers=max_workers)

            # 解析修复任务
            try:
                task_data = json.loads(args.get("fix_tasks", "[]"))
                for task in task_data:
                    fix_task = fixer.create_fix_task(
                        file_path=task["file_path"],
                        issue_type=task.get("issue_type", "unknown"),
                        description=task.get("description", ""),
                        priority=FixPriority[task.get("priority", "MEDIUM")],
                        old_code=task.get("old_code"),
                        new_code=task.get("new_code"),
                        line_number=task.get("line_number"),
                    )
                    fixer.add_task(fix_task)
            except json.JSONDecodeError as e:
                return {
                    "content": [{"type": "text", "text": f"JSON 解析错误: {str(e)}"}],
                    "is_error": True
                }

            # 执行修复
            results = await fixer.run_parallel_async(dry_run=dry_run)
        ...
        @tool(
            "auto_fix_all",
            "🚀 自动修复全部 - 完整检查前后端问题和日志错误，然后并行修复所有可自动修复的问题",
            {
                "check_frontend": bool,  # 是否检查前端
                "check_logs": bool,  # 是否检查日志
                "dry_run": bool,  # 是否仅模拟运行
                "max_workers": int,  # 最大工作线程数
            }
        )
        async def auto_fix_all(args: dict[str, Any]) -> dict[str, Any]:
            """自动检查并修复所有问题"""
            check_fe = args.get("check_frontend", True)
            check_logs = args.get("check_logs", True)
            dry_run = args.get("dry_run", True)  # 默认 dry_run 以安全为主
            max_workers = args.get("max_workers", 4)
            ...
            if check_fe:
                checker = FrontendChecker(project_path)
                fe_result = await checker.check_all()
                check_results["frontend"] = {
                    "typescript_errors": len(fe_result.typescript_errors),
                    "critical": len([e for e in fe_result.typescript_errors if e.severity.value == "critical"]),
                    "missing_dependencies": [d.module_name for d in fe_result.missing_dependencies],
                }

                # 从 TypeScript 错误生成修复任务
                task_gen = FixTaskGenerator(project_path)
                ts_tasks = task_gen.from_typescript_errors([
                    {
                        "code": e.code,
                        "file": e.file,
                        "line": e.line,
                    ...
                    for e in fe_result.typescript_errors
                ])
                all_tasks.extend(ts_tasks)

            # 2. 日志检查
            if check_logs:
                analyzer = LogAnalyzer(project_path)
                log_result = await analyzer.analyze(lines=500)
                check_results["logs"] = {
                    "total_errors": log_result.total_errors,
                    "critical": len(log_result.critical_errors),
                    "recommendations": log_result.recommendations,
                }

                # 从日志错误生成修复任务
                task_gen = FixTaskGenerator(project_path)
                log_tasks = task_gen.from_log_errors([e.to_dict() for e in log_result.errors])
                all_tasks.extend(log_tasks)

            # 3. 执行修复
            fix_summary = {"total": 0, "success": 0, "failed": 0}

            if all_tasks:
                fixer = ParallelFixer(project_path, max_workers=max_workers)
                fixer.add_tasks(all_tasks)
                ...
                if not dry_run:
                    for result in results:
                        if result.success:
                            await knowledge.add_fix({
                                "file": result.task.file_path,
                                "description": result.task.description,
                                "issue_type": result.task.issue_type,
                                "timestamp": datetime.now().isoformat(),
                            })
            ...
            output = {
                ...
                "message": "Dry run 完成，使用 dry_run=false 执行实际修复" if dry_run else f"修复完成：成功 {fix_summary['success']}，失败 {fix_summary['failed']}",
            }
    ...
    def _create_options(self) -> ClaudeAgentOptions:
        ...
        return ClaudeAgentOptions(
            ...
            allowed_tools=[
                ...
                # 前后端检查和并行修复工具
                "mcp__shencha__check_frontend",
                "mcp__shencha__analyze_pm2_logs",
                "mcp__shencha__parallel_fix",
                "mcp__shencha__auto_fix_all",
                # 内置工具
                "Read",
                "Write",
                "Edit",
...
这是你的核心能力，用于检测和修复代码问题：

1. **🔍 check_frontend** - 前端检查
   - 运行 TypeScript 类型检查 (pnpm type-check)
   - 运行 ESLint 代码规范检查
   - 检测缺失的 npm 依赖 (TS2307 错误)
   - 检测导入错误 (TS1361, TS2552 等)
   - 按严重程度分类错误 (critical, high, medium, low)

2. **📋 analyze_pm2_logs** - PM2 日志分析
   - 分析 PM2 错误日志
   - 检测运行时错误 (TypeError, ReferenceError 等)
   - 检测依赖问题 (sharp, 缺失模块等)
   - 检测数据库连接错误
   - 检测 API 错误 (Server Action 等)
   - 生成修复建议和可执行命令

3. **⚡ parallel_fix** - 并行修复
   - 多线程并行执行多个修复任务
   - 按优先级排序修复 (CRITICAL > HIGH > MEDIUM > LOW)
   - 自动备份文件
   - 失败时自动回滚
   - 支持 dry_run 模式预览修复

4. **🚀 auto_fix_all** - 自动全部修复
   - 一键运行所有检查 (前端 + 日志)
   - 自动生成修复任务
   - 并行执行所有修复
   - 默认 dry_run=true 以保证安全
   - 记录修复到知识库
...
**对于快速问题检测**:
- 使用 check_frontend 检测 TypeScript/ESLint 错误
- 使用 analyze_pm2_logs 检测运行时错误

**对于批量修复**:
- 使用 auto_fix_all (dry_run=true) 预览所有可修复问题
- 确认后使用 auto_fix_all (dry_run=false) 执行修复
